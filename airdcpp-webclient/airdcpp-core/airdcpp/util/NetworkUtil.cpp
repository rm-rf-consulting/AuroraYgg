...
bool NetworkUtil::isYggdrasilIp(const string& ip) noexcept {
	// Yggdrasil mesh network uses the 200::/7 IPv6 range
	// First byte is 0x02 or 0x03, so addresses start with "2" or "3" followed by hex digits and ":"
	// Common examples: 200:xxxx, 201:xxxx, 202:xxxx, 300:xxxx, 301:xxxx
	if (ip.length() < 4 || ip.find(':') == string::npos) {
		return false;
	}

	// Parse the first 16-bit group to check if it falls in 200::/7
	// 200::/7 means first 7 bits = 0000001x, i.e. first byte is 0x02 or 0x03
	// First group: 0x0200-0x03FF
	auto colonPos = ip.find(':');
	if (colonPos == 0 || colonPos > 4) {
		return false;
	}

	unsigned long firstGroup = strtoul(ip.substr(0, colonPos).c_str(), nullptr, 16);
	return firstGroup >= 0x0200 && firstGroup <= 0x03FF;
}
...
AdapterInfoList NetworkUtil::getNetworkAdapters(bool v6) {
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		if (ret == ERROR_SUCCESS)
		{
			...
			{
				// we want only enabled network interfaces (Ethernet, WiFi, and virtual/tunnel for Yggdrasil/WireGuard/VPN)
				if (pAdapterInfo->OperStatus == IfOperStatusUp && (
					pAdapterInfo->IfType == IF_TYPE_ETHERNET_CSMACD ||
					pAdapterInfo->IfType == IF_TYPE_IEEE80211 ||
					pAdapterInfo->IfType == IF_TYPE_PROP_VIRTUAL ||    // 53: Yggdrasil (WinTUN), Tailscale
					pAdapterInfo->IfType == IF_TYPE_TUNNEL ||          // 131: GRE/IP tunnels
					pAdapterInfo->IfType == IF_TYPE_PPP                // 23: PPP/VPN adapters
				))
				{
