...
void ConnectivityManager::detectConnection() {
	...
	if (detectV6) {
		auto localIpV6 = NetworkUtil::getLocalIp(true);
		if (NetworkUtil::isYggdrasilIp(localIpV6)) {
			// Yggdrasil mesh network: every node is directly routable, no UPnP needed
			{
				WLock l(cs);
				autoSettings[SettingsManager::INCOMING_CONNECTIONS6] = SettingsManager::INCOMING_ACTIVE;
				autoSettings[SettingsManager::EXTERNAL_IP6] = localIpV6;
			}

			log("Yggdrasil network detected (" + localIpV6 + "), using direct connection", LogMessage::SEV_INFO, TYPE_V6);
		} else if (NetworkUtil::isPublicIp(localIpV6, true)) {
			// Direct connection
			{
				WLock l(cs);
				autoSettings[SettingsManager::INCOMING_CONNECTIONS6] = SettingsManager::INCOMING_ACTIVE;
